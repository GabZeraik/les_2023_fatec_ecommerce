				  ...
				  <c:when test="${alunos != null && alunos.operacao == 'CONSULTAR'}">
					  <c:choose>
						  <c:when test="${alunos.dados.size() > 0}">
								        	...
								        	<div class="details-modal">
				            					...
				            					<div class="details-modal-content">
										            <form id="formulario_${item.id}" action="AlterarAluno" method="post" class="login-form">
										            	<label for="id_Aluno" >ID (não é possível alterar)</label>
										            	...
										            	<label for="cpf_Aluno">CPF (não é possível alterar)</label>
							...
							<c:otherwise>
					     		<p>Não há alunos cadastrados ou o parâmetro utilizado não retornou nenhum resultado.</p>
					  		</c:otherwise>
				  		</c:choose>
				  </c:when>
				  <c:otherwise>
...
<script type="text/javascript">
    ...
    htmlAluno = new TabelaHeader(['ID', 'Nome', 'CPF', 'Email', 'Data Nascimento', 'Registro Aluno', 'Endereço', 'Curso']).setHeaderTabela();
